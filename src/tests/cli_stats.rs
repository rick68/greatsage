...
#[cfg(test)]
mod tests {
    use super::super::*; // bring crate root into scope
    ...
    fn test_cli_stats_subcommand_outputs_version() {
        ...
        let info = crate::evolve::assessment_phase(std::path::Path::new(".")).expect("assessment_phase should succeed");
        // The result should contain a version line
        assert!(info.contains("Version:"), "Assessment info missing Version line: {}", info);
        // Also should contain Source files line
        assert!(info.contains("Source files:"), "Assessment info missing Source files line: {}", info);
    }
}
