...
mod help_data;
pub use help_data::cli_repl_commands_section;
...
use {
    ...
    dispatch::{dispatch_slash_command, unknown_command_message, DispatchResult},
    bevy::{
...
fn spawn_agent_reinstall(tokio_runtime: &mut TokioTasksRuntime, agent_config: AgentConfig) {
    ...
    tokio_runtime.spawn_background_task(|mut ctx| async move {
        ...
        ctx.run_on_main_thread(move |main_ctx| {
            () = crate::agents::install_coding_agent(
                main_ctx.world,
                coding_agent,
                model,
                provider,
        ...
        match *code {
            ...
            KeyCode::Enter if !content.is_empty() => {
                if content.trim_start().starts_with("/") {
                    match dispatch_slash_command(content.trim_start(), agent_config.as_mut()) {
                        DispatchResult::Exit => {
                            commands.remove_resource::<CodingAgentTask>();
                            exit.write_default();
                            stdout.write(StdoutMessage::newline());
