...
pub fn handle_global_input(
    ...
    for message in messages.read() {
        let KeyEvent { code, kind, modifiers, .. } = &**message;
        ...
        match code {
            ...
            KeyCode::Char('c') if matches!(kind, KeyEventKind::Press) && modifiers.contains(KeyModifiers::CONTROL) => {
                exit.write_default();
            }
            KeyCode::Up if !in_input && (kind == &KeyEventKind::Press || kind == &KeyEventKind::Repeat) => {
                actions.write(TuiAction::ScrollUp);
            }
            KeyCode::Down if !in_input && (kind == &KeyEventKind::Press || kind == &KeyEventKind::Repeat) => {
                actions.write(TuiAction::ScrollDown);
            }
            KeyCode::PageUp => { actions.write(TuiAction::ScrollUp); }
            KeyCode::PageDown => { actions.write(TuiAction::ScrollDown); }
            KeyCode::Home if !in_input => { actions.write(TuiAction::CursorToStart); }
            KeyCode::End if !in_input => { actions.write(TuiAction::CursorToEnd); }
...
pub fn handle_input_area_input(
    ...
    for message in messages.read() {
        ...
        match code {
            ...
            KeyCode::Backspace if is_active => { actions.write(TuiAction::Backspace); }
            KeyCode::Delete if is_active => { actions.write(TuiAction::Delete); }
            KeyCode::Left if is_active => { actions.write(TuiAction::CursorLeft); }
            KeyCode::Right if is_active => { actions.write(TuiAction::CursorRight); }
            KeyCode::Home if matches!(kind, KeyEventKind::Press) => { actions.write(TuiAction::CursorToStart); }
            KeyCode::End if matches!(kind, KeyEventKind::Press) => { actions.write(TuiAction::CursorToEnd); }
            KeyCode::Up if matches!(kind, KeyEventKind::Press) => { actions.write(TuiAction::HistoryPrev); }
            KeyCode::Down if matches!(kind, KeyEventKind::Press) => { actions.write(TuiAction::HistoryNext); }
            KeyCode::Enter if matches!(kind, KeyEventKind::Press) => { actions.write(TuiAction::Submit); }
            _ => (),
        }
    }
}
...
pub fn handle_mouse_input(
    ...
    use bevy_ratatui::crossterm::event::{MouseButton, MouseEventKind, MouseEvent};
    ...
    for message in messages.read() {
        ...
        match kind {
            MouseEventKind::ScrollUp => { actions.write(TuiAction::ScrollUp); }
            MouseEventKind::ScrollDown => { actions.write(TuiAction::ScrollDown); }
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if output_area.contains(Position { x: column, y: row }) {
                    ...
                    if let Some(Some((block_idx, click_action))) = tui_main.line_map.get(map_row).copied() {
                        match click_action {
                            crate::tui::core::ClickAction::ToggleThinking(ti) => {
                                actions.write(TuiAction::ToggleThinking { block_index: block_idx, thinking_index: ti });
                            }
