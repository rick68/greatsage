...
ANTHROPIC_API_KEY=sk-... cargo run -- --model claude-opus-4-7 --skills ./skills
```
...
greatsage constructs its prompts at runtime by loading the user‑defined `SkillSet` from the `./skills/` directory, injecting the current `ContextConfig` and the contents of `memory/active_learnings.md` into the system prompt via `build_dynamic_system_prompt()`. The tool list supplied to the LLM is generated by `build_tools()`, which enumerates all available `AgentTool` implementations and includes their concise descriptions. At each Bevy tick, the ECS world state (active resources, agent configuration, and temporary UI state) is queried and encoded into a short JSON fragment that is appended to the prompt, ensuring the LLM has up‑to‑date situational awareness.

The `evolve` skill monitors the outcomes of each interaction: token usage, test results, and user‑provided feedback. It analyses this history, synthesises refined prompt fragments, and safely validates them by running a dry‑run task within a sandboxed Bevy system. Successful refinements are committed to the `develop` branch, and the updated prompt logic is loaded automatically on the next evolution cycle. Throughout this process the TUI (implemented in `tui/tui_main.rs` with `bevy_ratatui`) displays real‑time progress, statistics, and any rollback decisions, giving the operator clear visibility into the self‑evolution workflow.
...
- Context loaded centrally by `scripts/greatsage_context.sh` → `$GREATSAGE_CONTEXT` (WHO YOU ARE, YOUR VOICE, SELF-WISDOM, SOCIAL WISDOM, YOUR ECONOMICS, YOUR SPONSORS sections)
...
[greatsage](https://github.com/rick68/greatsage) is built on [yoagent](https://github.com/yologdev/yoagent). Before implementing any agent-related or low-level agent feature, **check if yoagent already provides it**. Past examples of reinvented wheels:
...
3. If yoagent has it → use it. If yoagent almost has it → file an issue on yoagent. If yoagent doesn't have it → build it in greatsage.
