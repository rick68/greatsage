...
def select_issues(issues, sponsor_logins=None, pick=2, iteration=0):
    ...
    from the top 10 scored issues, seeded by iteration for reproducibility.
    """
    ...
    # Random pick from top 10 scored for remaining non-sponsor slots (seeded by iteration)
...
_bot_slug = os.environ.get("BOT_SLUG", "greatsage")
BOT_LOGINS = set(
...
def classify_issue(issue):
    ...
    Returns:
        "new" — greatsage never commented
        "human_replied" — human replied after greatsage's last comment
        "greatsage_last" — greatsage was last commenter, no new human replies
    """
    ...
    return "greatsage_last"


def format_issues(issues, sponsor_logins=None, pick=2, iteration=0):
    if not issues:
        return "No community issues today."

    # Classify each issue and split into active vs greatsage_last
    active = []
    greatsage_last = []
    for issue in issues:
        status = classify_issue(issue)
        issue["_status"] = status
        if status == "greatsage_last":
            greatsage_last.append(issue)
        else:
            active.append(issue)

    if not active and not greatsageo_last:
        return "No community issues today."
    ...
    greatsage_last.sort(key=score_key, reverse=True)

    # Select from active issues only; show greatsage_last only when nothing else is active
    if active:
        selected = select_issues(active, sponsor_logins, pick=pick, iteration=iteration)
    else:
        selected = greatsage_last[:pick]

    if not selected:
        return f"No new community issues (all {len(active) + len(greatsage_last)} already handled)."
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(format_issues(issues, sponsor_logins, pick=2, iteration=iteration))
    except (json.JSONDecodeError, FileNotFoundError):
        print("No community issues today.")