...
#   REPO               — GitHub repo (default: rick68/greatsage)
...
#   FALLBACK_PROVIDER  — Fallback provider on API error (e.g., "zai"); passed as --fallback to greatsage
...
# ITERATION_COUNT is written at the end of the session (separate commit, immune to task reverts)
...
if [ -f scripts/greatsage_context.sh ]; then
    source scripts/greatsage_context.sh
else
    echo "WARNING: scripts/greatsage_context.sh not found — prompts will lack identity context" >&2
    GREATSAGE_CONTEXT=""
fi
...
# Run greatsage with optional --fallback flag for provider failover.
...
if command -v gh &>/dev/null; then
    ...
    python3 scripts/format_issues.py /tmp/issues_raw.json "$SPONSOR_INFO_FILE" "$ITERATION" > "$ISSUES_FILE" 2>"$FORMAT_STDERR" || echo "No issues found." > "$ISSUES_FILE"
...
# Fetch greatsage's own backlog (agent-self issues)
SELF_ISSUES=""
if command -v gh &>/dev/null; then
...
# Fetch pending replies on all labeled issues (greatsage commented, human replied after)
PENDING_REPLIES=""
if command -v gh &>/dev/null; then
...
for issue in data:
    ...
    last_greatsage_idx = -1
    for i, c in enumerate(comments):
        author = (c.get('author') or {}).get('login', '')
        if author == bot_login:
            last_greatsage_idx = i

    if last_greatsage_idx == -1:
        continue  # bot never commented on this issue

    # Check for human replies after bot's last comment
    human_replies = []
    for c in comments[last_greatsage_idx + 1:]:
        author = (c.get('author') or {}).get('login', '')
        if author != bot_login:
            body = c.get('body', '')[:300]
...
$(cat "session_plan/eval_task_${TASK_NUM}.md" 2>/dev/null || echo 'no eval file available')"
    ...
    if [ "$TASK_OK" = false ]; then
        ...
        if command -v gh &>/dev/null; then
            ...
            ISSUE_BODY="**Iteration $ITERATION, Task $TASK_NUM** was automatically reverted by the verification gate.
...
$TASK_DESC"
            ...
            if [ -n "$EXISTING_ISSUE" ]; then
                if gh issue comment "$EXISTING_ISSUE" --repo "$REPO" \
                    --body "Reverted again on Iteration $ITERATION. Reason: $REVERT_REASON
...
- ${t:-unknown task}"
        ...
        PLAN_ISSUE_BODY="All tasks planned on Iteration $ITERATION were reverted. No code shipped.
...
**Action for next session:** Focus on smaller, more incremental changes. Consider breaking these tasks into sub-tasks that can each pass verification independently."
        ...
        gh issue create --repo "$REPO" \
            --title "Planning-only session: all $TASK_NUM tasks reverted (Iteration $ITERATION)" \
...
if ! grep -q "## Iteration $ITERATION.*$SESSION_TIME" journals/JOURNAL.md 2>/dev/null; then
    echo "  No journal entry found — running agent to write one..."
    COMMITS=$(git log --oneline "$SESSION_START_SHA"..HEAD --format="%s" | grep -v "session wrap-up\|cargo fmt" | sed "s/Iteration $ITERATION[^:]*: //" | paste -sd ", " - || true)
    if [ -z "$COMMITS" ]; then
...
"
    ...
    # journals/JOURNAL.md before. Used to prompt greatsage to write a first-time
...
if [ "$ISSUE_COUNT" -gt 0 ] && command -v gh &>/dev/null; then
    ...
    while IFS= read -r check_num; do
        ...
        if echo "$LAST_COMMENT" | grep -q "Iteration $ITERATION"; then
...
Commands:
- Comment: gh issue comment NUMBER --repo $REPO --body "🔮 **Iteration $ITERATION**
...
- Write in greatsage's voice — curious, honest, celebratory. No corporate speak.
RESPONDEOF
...
# Update ITERATION_COUNT (separate commit — immune to task reverts)
