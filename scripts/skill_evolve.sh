...
#   FALLBACK_PROVIDER            — passed through to greatsage as --fallback
...
if [ "$DRY_RUN" != "true" ]; then
    ...
    [ -x "$GREATSAGE_BIN" ] || { echo "skill-evolve: $GREATSAGE_BIN missing"; exit 1; }
else
    GREATSAGE_BIN="./target/debug/greatsage"  # set anyway for downstream env consistency
fi
...
if git fetch --depth 100 origin audit-log:audit-log 2>/dev/null; then
    if git worktree add "$AUDIT_WT" audit-log 2>/dev/null; then
        export GREATSAGE_AUDIT_DIR="$AUDIT_WT/sessions"
        echo "skill-evolve: audit evidence at $GREATSAGE_AUDIT_DIR ($(ls "$GREATSAGE_AUDIT_DIR" 2>/dev/null | wc -l) sessions)"
    fi
fi
...
\$GREATSAGE_AUDIT_DIR = ${GREATSAGE_AUDIT_DIR:-(unavailable — no audit-log branch yet)}
Run \`ls "\$GREATSAGE_AUDIT_DIR" | tail -30\` and read individual session files there for fine-grained tool-call evidence.
...
if [ "$HEAD_BEFORE" != "$HEAD_AFTER" ]; then
    ...
    while IFS= read -r f; do
        ...
        case "$f" in
            ...
            skills/*/SKILL.md)
                # Per-file check: must be a greatsage-origin skill, not core, not skill-evolve itself.
                ...
                if ! grep -q "^origin: greatsage$" "$f" 2>/dev/null; then
                    VIOLATIONS="${VIOLATIONS}  - HARD RULE #1 violation: $f lacks 'origin: greatsage' (not eligible)\n"
                    continue
                fi
                ;;
            *)
