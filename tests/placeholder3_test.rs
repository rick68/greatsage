// Test that executing Placeholder Task 3 creates the appropriate marker file.
...
mod tests {
    ...
    #[test]
    fn placeholder_task_3_creates_marker() {
        ...
        let marker_path = base.join(".greatsage").join("placeholder3.txt");
        assert!(marker_path.is_file(), "placeholder3.txt should exist");
        let content = fs::read_to_string(&marker_path).expect("read marker file");
        assert_eq!(content, "Task 3 completed");
    }
}
